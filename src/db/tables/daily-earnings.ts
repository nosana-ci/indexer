import { pgTable, varchar, numeric, serial, date, unique } from "drizzle-orm/pg-core";
...
export const dailyEarnings = pgTable(
  ...
  (table) => ({
    dailyEarningsUnique: unique("daily_earnings_unique").on(table.date, table.node, table.market),
  }),
);
