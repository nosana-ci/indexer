...
export const jobs = pgTable(
  'jobs',
  {
    ...
    address: varchar('address', { length: MAX_PUBKEY_LENGTH })
      .notNull()
      .unique(),
    ...
    benchmarkProcessedAt: timestamp('benchmark_processed_at'),
    timeout: integer('timeout').notNull().default(7200),
    usdRewardPerHour: real('usd_reward_per_hour'),
    listedAt: integer('listed_at'),
  },
  (table) => ({
    stateTimeStartIdx: index('idx_jobs_state_timestart').on(
      table.state,
      desc(table.timeStart)
    ),
  })
);
