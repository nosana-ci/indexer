...
export async function getNosPrice(
  ...
  try {
    ...
    const fallbackPrice = await getCachedNosPrice(
      targetTimestamp,
      12 * 60 * 60 * 1000
    );
    if (fallbackPrice !== null) {
      console.warn(
        `Using fallback cached price for timestamp ${targetTimestamp.toISOString()}`
      );
      return fallbackPrice;
    }

    console.error(
      `No NOS price available for timestamp ${targetTimestamp.toISOString()}`
    );
    return null;
  } catch (error) {
...
async function getCachedNosPrice(
  ...
  try {
    ...
    const result = await db
      ...
      .orderBy(
        sql`ABS(EXTRACT(EPOCH FROM ${stats.date}) - ${targetTimestamp.getTime() / 1000})`
      )
      .limit(1)
      .execute();
    ...
    const timeDiff = Math.abs(
      result[0].date.getTime() - targetTimestamp.getTime()
    );
